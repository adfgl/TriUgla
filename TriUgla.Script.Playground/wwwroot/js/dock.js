window.dockInterop = {
    ...
    initialize(workspaceSelector) {
        ...
        if (!layout || layout.dataset.dockInitialized)
            return;
        ...
        this.renderAllTabs();
        layout.querySelectorAll("[data-resize-dock]").forEach(splitter => this.initializeDockSplitter(splitter, splitter.dataset.resizeDock));
    },
    host(name) {
    ...
    applyDefaultLayout() {
        ...
        ["--left-width", "--right-width", "--top-height", "--bottom-height"].forEach(property => this.layout.style.removeProperty(property));
    },
    renderAllTabs() {
        this.dockNames.forEach(name => this.renderTabs(name));
        this.dockNames.forEach(name => this.layout.classList.toggle(`has-${name}`, this.panels(name).length > 0));
        this.layout.classList.remove("fill-center-left", "fill-center-right", "fill-center-top", "fill-center-bottom");
        if (this.panels("center").length === 0) {
            const fillDock = ["left", "right", "top", "bottom"]
                .find(name => this.panels(name).length > 0);
            if (fillDock)
                this.layout.classList.add(`fill-center-${fillDock}`);
        }
    ...
    renderTabs(name) {
        ...
        panels.forEach(panel => {
            ...
            panel.id || (panel.id = `dock-panel-${id}`);
    ...
    startPointerDrag(event, id, title) {
        if (event.button !== 0 || this.pointerDrag)
            return;
        ...
        const move = moveEvent => {
            if (moveEvent.pointerId !== pointerId || !this.pointerDrag)
                return;
        ...
        const stop = stopEvent => {
            ...
            window.setTimeout(() => {
                if (this.suppressClickId === drag?.id)
                    this.suppressClickId = null;
            }, 0);
        };
    ...
    dropDestinationAt(x, y, draggedId) {
        ...
        for (const name of this.dockNames) {
            const host = this.host(name);
            const tabs = host?.querySelector(":scope > .dock-tabs");
            if (!tabs || getComputedStyle(host).visibility === "hidden")
                continue;
            const tabBounds = tabs.getBoundingClientRect();
            if (x < tabBounds.left || x > tabBounds.right || y < tabBounds.top || y > tabBounds.bottom)
                continue;
    ...
    showDropDestination(destination) {
        ...
        this.layout.querySelectorAll(".dock-tab-insert, .dock-tabs-insert-end").forEach(item => item.classList.remove("dock-tab-insert", "dock-tabs-insert-end"));
        if (!destination?.dock)
            return;
    ...
    restore() {
        ...
        if (state.leftWidth)
            this.layout.style.setProperty("--left-width", state.leftWidth);
        if (state.rightWidth)
            this.layout.style.setProperty("--right-width", state.rightWidth);
        if (state.topHeight)
            this.layout.style.setProperty("--top-height", state.topHeight);
        if (state.bottomHeight)
            this.layout.style.setProperty("--bottom-height", state.bottomHeight);
    }
};
