...
public sealed class Mesh(Face root) : IMeshTraversal
{
        ...
        CanTraverseAcrossEdge? canTraverse = null)
    {
        ...
        {
            if (edge.NodeStart.TryVisit(stamp))
            {
                yield return edge.NodeStart;
            }
        }
    }
        ...
        CanTraverseAcrossEdge? canTraverse = null)
    {
        ...
        foreach (Face face in Faces(from, canTraverse))
        {
            ...
            {
                edges.Add(edge);
                if (edge.NodeStart.TryVisit(nodeStamp))
                {
                    nodes.Add(edge.NodeStart);
                }
            }
        }
    ...
    public void ResetVisitStamps()
    {
        ...
        while (stack.TryPop(out Face? face))
        {
            ...
            foreach (Edge edge in face.Edges)
            {
                ...
                if (neighbour is not null && visitedFaces.Add(neighbour))
                {
        ...
        CanTraverseAcrossEdge? canTraverse)
    {
        foreach (Edge edge in face.Edges)
        {
            ...
            if (canTraverse is not null && !canTraverse(face, edge, neighbour))
            {
                continue;
            }

            if (neighbour.TryVisit(stamp))
            {
                stack.Push(neighbour);
            }
        }
